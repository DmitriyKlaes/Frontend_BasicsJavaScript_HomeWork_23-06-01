...
const moreThenThree = array => array.filter(item => item >= 3);

console.log(moreThenThree(Object.values(numbers)));
// или сразу в консоль
...
products.forEach(element => element.price = Math.round(element.price *= 0.85));
...
console.log(productsFour.filter(object => object.hasOwnProperty('photos') && object.photos.length != 0));

productsFour.sort((a, b) => a.price - b.price);
console.log(productsFour);
...
const translatorWeekDays = en.reduce((prev, elem, index) => (prev[elem] = ru[index], prev), {});
console.log(translatorWeekDays);